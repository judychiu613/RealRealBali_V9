    ...
    const processProperty = async (property: any) => {
      ...
      return {
        ...
        landZone: property.land_zone ? {
          en: property.land_zone.name_en,
          zh: property.land_zone.name_zh
        } : null,
        // 标签处理 - 根据语言返回对应标签
        tags: tags || [],
    ...
    switch (endpoint) {
      case 'properties': {
        ...
        const { data: properties, error } = await supabaseClient
          ...
          .select(`
            *,
            area:property_areas_final_2026_02_21_17_30!inner(name_en, name_zh),
            land_zone:property_land_zones_final_2026_02_21_17_30(name_en, name_zh),
            property_types_final_2026_02_22_09_30!inner(name_en, name_zh)
          `)
        ...
        const processedProperties = await Promise.all(
          (properties || []).map(async (property) => {
            // 添加类型名称
            const typeInfo = property.property_types_final_2026_02_22_09_30;
            property.type_name_en = typeInfo?.name_en || 'Villa';
            property.type_name_zh = typeInfo?.name_zh || '别墅';
