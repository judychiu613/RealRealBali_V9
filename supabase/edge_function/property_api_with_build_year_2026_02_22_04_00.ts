...
async function getPropertyById(supabaseClient: any, propertyId: string) {
  ...
  await recordPropertyView(supabaseClient, propertyId)
  ...
  return new Response(JSON.stringify({ data: transformedProperty }), {
    headers: { ...corsHeaders, 'Content-Type': 'application/json' }
  })
}
...
async function getSimilarProperties(supabaseClient: any, propertyId: string) {
  // First get the base property to find similar ones
  const { data: baseProperty, error: baseError } = await supabaseClient
    .from('properties_final_2026_02_21_17_30')
...
async function getPropertyStats(supabaseClient: any) {
  const { data, error } = await supabaseClient
    .from('properties_final_2026_02_21_17_30')
    .select('price_usd, bedrooms, land_area, area_id, type_id')
    .eq('is_published', true)
  ...
  const stats = {
    total: data.length,
    avgPrice: data.reduce((sum, p) => sum + (p.price_usd || 0), 0) / data.length,
    minPrice: Math.min(...data.map(p => p.price_usd || 0)),
    maxPrice: Math.max(...data.map(p => p.price_usd || 0)),
    avgBedrooms: data.reduce((sum, p) => sum + (p.bedrooms || 0), 0) / data.length,
    avgLandArea: data.reduce((sum, p) => sum + (p.land_area || 0), 0) / data.length
  }
