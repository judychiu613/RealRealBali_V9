  ...
  try {
    ...
    if (path === 'categories') {
      const { data: categories, error } = await supabaseClient
        .from('property_categories_2026_02_22_08_00')
        .select('*')
        .order('sort_order');
    ...
    // Get subcategories endpoint
    if (path === 'subcategories') {
      const categoryId = url.searchParams.get('category_id');
      
      let query = supabaseClient
        .from('property_subcategories_2026_02_22_08_00')
        .select('*');
    ...
    if (path === 'property') {
      ...
      const { data: property, error } = await supabaseClient
        ...
        .select(`
          *,
          ...
          type:property_types_final_2026_02_21_17_30(name_en, name_zh),
          landZone:land_zones_2026_02_22_06_00(name_en, name_zh),
          category:property_categories_2026_02_22_08_00(id, name_en, name_zh),
          subcategory:property_subcategories_2026_02_22_08_00(id, name_en, name_zh)
        `)
        .eq('id', propertyId)
        .single();
      ...
      const processedProperty = {
        ...
        category: property.category ? {
          ...
          en: property.category.name_en,
          zh: property.category.name_zh
        } : null,
        subcategory: property.subcategory ? {
          id: property.subcategory.id,
          en: property.subcategory.name_en,
          zh: property.subcategory.name_zh
        } : null,
