...
serve(async (req) => {
  ...
  try {
    ...
    const processImagePath = (imagePath: string): string => {
      ...
      // Convert database path to public path
      if (imagePath.startsWith('/src/assets/images/')) {
        return imagePath.replace('/src/assets/images/', '/images/');
      }
      return imagePath;
    };

    // Helper function to process property data with correct field mapping
    const processProperty = (property: any) => {
      const mainImage = processImagePath(property.main_image);
      const imageArray = property.images ? 
        (Array.isArray(property.images) ? property.images.map(processImagePath) : [processImagePath(property.images)]) : 
        [mainImage];
