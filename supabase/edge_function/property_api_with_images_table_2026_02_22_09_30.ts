...
serve(async (req) => {
  ...
  try {
    ...
    // Helper function to process property data with images from property_images table
    const processProperty = async (property: any) => {
      ...
      const mainImage = imageUrls.length > 0 ? imageUrls[0] : '/images/default-property.jpg';
    ...
    if (path === 'properties' || path === 'featured') {
      ...
      // Process properties data (with images from property_images table)
