  ...
  try {
    ...
    const getPropertyImages = async (propertyId: string) => {
      try {
        ...
        console.error('Exception in getPropertyImages:', error);
        return [];
      }
    };
    ...
    const processProperty = async (property: any) => {
      try {
        ...
        const mainImage = imageUrls[0] || property.image || 'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=800&h=600&fit=crop';
