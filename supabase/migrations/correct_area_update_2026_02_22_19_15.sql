-- 根据前端LOCATION_FILTERS定义，将房源从一级区域更新为二级区域
-- 为每个一级区域的房源随机分配其对应的二级区域

-- 更新水明漾(seminyak)的房源到其二级区域
