-- 1. 临时移除外键约束（如果存在）
ALTER TABLE properties DROP CONSTRAINT IF EXISTS properties_area_id_fkey;
ALTER TABLE properties DROP CONSTRAINT IF EXISTS properties_type_id_fkey;

-- 2. 检查当前数据
SELECT COUNT(*) as current_properties FROM properties;

-- 3. 清理可能存在的测试数据
DELETE FROM property_images WHERE property_id LIKE 'test-%';
