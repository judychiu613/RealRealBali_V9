...
// Create new property
async function createProperty(supabase: any, body: any, userId: string) {
  try {
    const {
      title_zh,
      title_en,
    ...
    if (!title_zh || !title_en || !price_usd) {
      return new Response(
    ...
    const { data: property, error } = await supabase
      ...
      .insert({
        ...
        type_id: type_id || 'villa',
        area_id: area_id || 'seminyak',
        price_usd: Number(price_usd),
        price_cny: Number(price_cny) || 0,
        price_idr: Number(price_idr) || 0,
        bedrooms: Number(bedrooms) || 0,
        bathrooms: Number(bathrooms) || 0,
        building_area: Number(building_area) || 0,
...
async function getPropertyDetail(supabase: any, propertyId: string, userId: string, isSuperAdmin: boolean) {
  return new Response(
    JSON.stringify({ success: false, error: '功能开发中' }),
    { status: 501, headers: corsHeaders }
  );
}

async function updateProperty(supabase: any, body: any, userId: string, isSuperAdmin: boolean) {
  return new Response(
    JSON.stringify({ success: false, error: '功能开发中' }),
    { status: 501, headers: corsHeaders }
  );
...
async function deleteProperty(supabase: any, body: any, userId: string, isSuperAdmin: boolean) {
  return new Response(
    JSON.stringify({ success: false, error: '功能开发中' }),
    { status: 501, headers: corsHeaders }
  );
